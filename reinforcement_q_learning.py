...
def plot_durations():
    ...
    plt.xlabel('Time (s)')
    plt.ylabel('Score')
    plt.plot(episode_durations, durations_t.numpy())
...
if True:
    ...
    for i_episode in count():
        ...
        if time.time() - start_time >= TIME_LIMIT:
            save_agent(best[0], "dqn_best_policy.pth")
            save_agent(best[1], "dqn_best_target.pth")
            break
        env.reset()
        ...
        for t in count():
            ...
            optimize_model()
            if done:
                episode_scores.append(sum_)
                episode_durations.append(time.time() - start_time)
                if sum_ > best_score:
                    best = (policy_net, target_net)
                    best_score = sum_
                plot_durations()
                break
    ...
    with open("dqn_data.txt", "w") as file:
        ...
        file.write("\n")
        file.write(",".join(map(str, episode_durations)))
